...
- Task 7 amended post-review (commit ee453e8): answered-prayer log backfill uses local dates via `todayStr(new Date(answeredAt))` (was UTC slice); importLegacy is idempotent (upsert with ignoreDuplicates on both tables) and re-runnable — Task 9's hydration should call `importLegacy` whenever a legacy snapshot exists (after a pre-fetch for the seeded categories), then a refetch — two fetches total when a snapshot exists, one otherwise.
- Task 8 amended post-review (commit d3337d9): useSession is purely event-driven (onAuthStateChange incl. INITIAL_SESSION; no getSession call); SignIn has a pending "Redirecting…" state; test mocks require vi.hoisted() wrapping under Vitest 4.
- Task 9 note from reviews: write-through must SERIALIZE writes in dispatch order (promise chain), so a new category's insert lands before a prayer referencing it (FK on prayers.category_id); pre-account 'local'-keyed dev caches are consciously dropped, not migrated.
- Task 11 amended: DRY'd the cache prefix into an exported `CACHE_PREFIX` (persistence.ts + Home sign-out sweep, from Task 10 review); removed dead `isYesterday` helper and its tests; SKILL.md corrected post-review — missing env vars crash at module load (blank page), only the pre-OAuth state shows the sign-in screen; legacy-seeding recipe restored.
